...
export default function SiteBackgroundNetwork() {
  ...
  useEffect(() => {
    ...
    const createPoints = () => {
      ...
      points = Array.from({ length: count }, () => {
        ...
        return {
          ...
          originX: x,
          originY: y,
          velocityX: (seededRandom() - 0.5) * 0.12,
          velocityY: (seededRandom() - 0.5) * 0.12,
          depth: 0.35 + seededRandom() * 0.7,
          size: 0.55 + seededRandom() * 0.85,
        };
      });
    };
    ...
    const drawNetwork = () => {
      ...
      for (let firstIndex = 0; firstIndex < points.length; firstIndex += 1) {
        ...
        for (let secondIndex = firstIndex + 1; secondIndex < points.length; secondIndex += 1) {
          ...
          context.strokeStyle = cursorEmphasis > 0.05
            ? `rgba(216, 32, 43, ${alpha * 0.78})`
            : `rgba(213, 221, 228, ${alpha * 0.42})`;
      ...
      for (const point of points) {
        ...
        context.fillStyle = cursorEmphasis > 0.08
          ? `rgba(229, 50, 61, ${0.2 + cursorEmphasis * 0.5})`
          : "rgba(226, 232, 237, 0.18)";
        context.fill();
      }
    };
    ...
    const handleVisibilityChange = () => {
      ...
      if (isDocumentVisible && !staticExperience && !animationFrame) {
