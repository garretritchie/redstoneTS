...
import { ArrowUpRight, Brain, Briefcase, ChartLineUp, Compass, FlowArrow, GearSix, HardDrives, MagnifyingGlass, SealCheck, TrendUp, Wrench } from "@phosphor-icons/react/dist/ssr";
...
export const metadata: Metadata = {
  title: "Technology Projects, AI, Compliance & Automation",
  description: "Redstone delivers technology projects, security compliance, AI-powered web and application development, workflow automation and technology strategy.",
...
export default function CapabilitiesPage() {
  return (
    <main id="top">
      <SiteHeader />
      <PageHero eyebrow="Beyond managed services" title={<>Build what is next.<br /><span>Improve what exists.</span></>} description="Redstone brings specialist project delivery, security governance, intelligent software, automation and strategic guidance under the same accountable relationship." secondaryLabel="See how we work" secondaryHref="#process" />
      <section className="capabilities-section section-pad" aria-labelledby="solutions-title">
        <div className="section-shell"><div className="capabilities-heading"><p className="eyebrow">Specialist capabilities</p><h2 id="solutions-title">Serious expertise.<br />Applied to business outcomes.</h2><p>Engage Redstone for a single initiative or as part of a longer technology roadmap. Either way, the work stays connected to your operation.</p></div><div className="capabilities-list">{capabilities.map(({ name, text, icon: Icon }, index) => <article key={name}><span>0{index + 1}</span><Icon size={31} weight="light" aria-hidden="true" /><div><h3>{name}</h3><p>{text}</p></div><a href="/contact" aria-label={`Discuss ${name}`}>Discuss this capability <ArrowUpRight size={17} weight="bold" aria-hidden="true" /></a></article>)}</div></div>
      </section>
