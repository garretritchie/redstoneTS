...
import { CheckCircle, ChatCircleText, Heart, Headset, ShieldCheck, UserFocus } from "@phosphor-icons/react";
import AssessmentBand from "../components/AssessmentBand";
import AboutRedstoneDetails from "../components/AboutRedstoneDetails";
...
const technologyPartners = [
  ["Microsoft", "Cloud & productivity"], ["Sophos", "Cybersecurity"], ["Cisco", "Networking & infrastructure"], ["Juniper", "Networking & infrastructure"], ["Veeam", "Backup & resilience"], ["Wasabi", "Cloud storage"], ["CompTIA", "Professional ecosystem"], ["Dell", "Business infrastructure"], ["HP", "Business infrastructure"], ["VMware", "Virtualisation"], ["Digium", "Business communications"], ["Asterisk", "Business communications"], ["Ubiquiti", "Networking & connectivity"],
];
...
export default function AboutPage() {
  ...
  return (
    <main id="top">
      <Helmet>
        <title>About Redstone Technology Solutions</title>
        <meta name="description" content="Meet Redstone, a Bahamian technology partner built around proactive service, accountable relationships, experienced leadership and human support." />
        <link rel="canonical" href="https://www.redstonets.com/about" />
      </Helmet>
      <SiteHeader />
      ...
      <section className="partnerships-section section-pad" id="partnerships" aria-labelledby="partnerships-title">
        <div className="section-shell partnerships-layout">
          <div className="partnerships-intro"><p className="eyebrow">Technology partners &amp; affiliations</p><h2 id="partnerships-title">Proven technology.<br /><span>One accountable partner.</span></h2><p>We are proud to be registered and affiliated with organisations across cloud, cybersecurity, infrastructure, networking, continuity and professional development.</p><p className="partnerships-note">This broad ecosystem helps Redstone recommend and support the right-fit technology for each client environment\u2014without forcing every business into the same stack.</p></div>
          <div className="partnerships-grid" aria-label="Redstone technology partners and affiliations">
            {technologyPartners.map(([name, category], index) => <article key={name}><span>{String(index + 1).padStart(2, "0")}</span><h3>{name}</h3><p>{category}</p></article>)}
            <div className="partnerships-summary"><span>Connected by Redstone</span><p>One integrated environment.<br />One accountable team.</p></div>
          </div>
        </div>
      </section>
