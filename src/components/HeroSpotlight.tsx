  ...
  useEffect(() => {
    ...
    const makePoints = () => {
      ...
      points = Array.from({ length: count }, () => {
        ...
        return { x, y, originX: x, originY: y, velocityX: (seededRandom() - 0.5) * 0.18, velocityY: (seededRandom() - 0.5) * 0.18, depth: 0.35 + seededRandom() * 0.75, size: 0.75 + seededRandom() * 1.35 };
      });
    };
    ...
    const drawNetwork = () => {
      ...
      for (let firstIndex = 0; firstIndex < points.length; firstIndex += 1) {
        ...
        for (let secondIndex = firstIndex + 1; secondIndex < points.length; secondIndex += 1) {
          ...
          context.strokeStyle = cursorEmphasis > 0.08 ? `rgba(190, 31, 43, ${alpha})` : `rgba(117, 42, 49, ${alpha * 0.72})`;
      ...
      for (const point of points) {
        ...
        context.fillStyle = cursorEmphasis > 0.06 ? `rgba(216, 32, 43, ${0.28 + cursorEmphasis * 0.48})` : "rgba(142, 45, 54, 0.34)";
        context.fill();
      }
    };
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      pendingMouseX = event.clientX - rect.left;
      pendingMouseY = event.clientY - rect.top;
      if (!pointerFrame) {
        pointerFrame = window.requestAnimationFrame(() => { mouseX = pendingMouseX; mouseY = pendingMouseY; pointerFrame = 0; });
      }
    };

    const handleMouseEnter = () => { isHovering = true; canvas.classList.add("is-active"); };
    const handleMouseLeave = () => { isHovering = false; canvas.classList.remove("is-active"); };
