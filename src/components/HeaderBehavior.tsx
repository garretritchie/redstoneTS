...
export default function HeaderBehavior() {
  useEffect(() => {
    ...
    const measure = () => { heroBottom = hero.getBoundingClientRect().bottom + window.scrollY; };
    const update = () => { frame = 0; header.classList.toggle("is-fixed", window.scrollY >= heroBottom); };
    ...
    const handleMobileNavToggle = () => {
      mobileNavSummary?.setAttribute("aria-label", mobileNav?.open ? "Close navigation" : "Open navigation");
    };
    const handleMobileNavClick = (event: Event) => { if ((event.target as HTMLElement).closest("a")) closeMobileNav(); };
    const handleDocumentPointerDown = (event: PointerEvent) => { if (mobileNav?.open && !mobileNav.contains(event.target as Node)) closeMobileNav(); };
    const handleEscape = (event: KeyboardEvent) => { if (event.key === "Escape" && mobileNav?.open) { closeMobileNav(); mobileNavSummary?.focus(); } };
